...
public partial class Admin_User_Management_Add_Vendor : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        txtAccountNumber.Text = h.GenerateAccountNumber("tbl_vendors", "VD");
    }
    ...
    #region FILL BRANDS DROPDOWN

    public void FillBrandsDropdown()
    {
        DataSet ds = new DataSet();
        using (SqlConnection conn = new SqlConnection(Database.connection))
        {
            try
            {
                string qry = "SELECT Name FROM tbl_brands ORDER BY Name ASC";
                SqlCommand cmd = new SqlCommand(qry, conn);
                conn.Open();
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                sda.Fill(ds);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    drpBrand.DataSource = ds.Tables[0];
                    drpBrand.DataBind();
                }
            }
            catch (Exception ex)
            {
                popupDanger.Visible = true;
                errMessage.InnerHtml = h.ErrMessage("Brands", ex.Message, "Error Filling DropDown");
            }
            finally
            {
                conn.Close();
    ...
    #endregion FILL BRANDS DROPDOWN
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            ...
            if (insert == true)
            {
                Clear();
                popup.Visible = true;
                message.InnerHtml = h.SuccessSaved("Vendor");
            }
            else
            {
                popupDanger.Visible = true;
                errMessage.InnerHtml = h.ErrorSaved("Vendor", v.Error);
            }
        }
    }
    ...
    {
        Response.Redirect("~/Admin/User-Management/All-Vendors.aspx");
    }
    ...
    public void Clear()
    {
        txtName.Text = txtEmail.Text = txtPhone.Text = txtPhone2.Text = txtAddress.Text = txtState.Text = txtPincode.Text = txtGST.Text = txtLicense.Text = txtAadhar.Text = "";
